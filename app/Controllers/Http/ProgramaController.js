 ...
 * tags:
 *   name: Sistemas
 *   description: API para la gestión de programas
 */
  ...
  async index({ request, response }) {
    if (connection === 'mongodb') {
      ...
      try {
        ...
        const programas = database.collection('programas');
      ...
      const total = await programas.countDocuments(filter);
      const cursor = programas.find(filter)
                              .sort(sort)
                              .skip(skip)
                              .limit(limit);
        const programasRes = await cursor.toArray();
      ...
      const programasRes = await query.paginate(page, limit);
      return response.json(programasRes);
    }
  }
  ...
  async show({ params, response }) {
    if (connection === 'mongodb') {
      ...
      try {
        ...
        const programa = await programas.findOne({ id: params.id });
      ...
      const programa = await Programa.find(params.id);
      return response.json(programa);
    }
  }
  ...
  async store({ request, response }) {
    ...
    if (connection === 'mongodb') {
      ...
      try {
        ...
        const database = client.db('marcos');
        const programas = database.collection('programas');
        const result = await programas.insertOne(data);
      ...
      const programa = await Programa.create(data);
      return response.status(201).json(programa);
    }
  }
  ...
  async destroy({ params, response }) {
    if (connection === 'mongodb') {
      ...
      try {
        ...
        const database = client.db('marcos');
        const programas = database.collection('programas');
        await programas.deleteOne({ id: params.id });
